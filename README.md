...
- 🚀 **快捷键操作** - 所有操作都可通过快捷键完成
- 🧩 **智能去重** - 自动识别并合并装饰型标题（如 `===` 分隔块包裹的章节标题）与正文重复的纯文本标题，避免目录中出现重复条目
...
| 隐藏文本 | `Ctrl+→` | `Cmd+→` | 隐藏小说内容（老板模式） |
| 选择章节 | `Ctrl+Alt+C` | `Cmd+Alt+C` | 打开章节选择器 |

> 💡 所有快捷键都可以在 VS Code 的 `文件 → 首选项 → 键盘快捷方式` 中自定义
>
> ⚠️ `Ctrl+←` 和 `Ctrl+→` 默认用于光标按单词跳转，`Ctrl+↓` 默认用于滚动行。本插件会占用这些快捷键，如需保留原功能请在设置中改为带 Alt 修饰键（如 `Ctrl+Alt+←`）
...
- **类型**：字符串
- **默认值**：`^\s*第[0-9一二三四五六七八九十百千零]+[章回节].*$`
- **说明**：用于匹配章节标题的正则表达式（默认支持带前导空白的装饰型标题）
- **示例**：
  - 匹配 "第一章"、"第二回"：`^\s*第[0-9一二三四五六七八九十百千零]+[章回节].*$`
...
### Q: 为什么章节解析不出来？
A: 检查 `novelReader.chapterRegex` 配置。默认正则为 `^\s*第[0-9一二三四五六七八九十百千零]+[章回节].*$`，已支持带前导空白的装饰型标题（如 `===` 分隔块包裹的章节）。如果你的小说章节格式特殊（如 "Chapter 1"、"1. 标题"），请相应修改正则表达式。

### Q: 章节目录出现重复条目怎么办？
A: 插件已内置去重逻辑，会自动合并装饰版标题与相邻纯文本版标题（距离小于 200 字符）。如果仍有重复，可能是整章内容重复（相距较远），需要手动编辑小说文件删除重复章节。
...
### Q: 快捷键冲突怎么办？
A: 本插件的 `Ctrl+←`、`Ctrl+↓`、`Ctrl+→` 会占用 VS Code 默认的单词跳转和行滚动快捷键。如需保留原功能，请在 `文件 → 首选项 → 键盘快捷方式` 中自定义，或将本插件的快捷键改为带 Alt 修饰键（如 `Ctrl+Alt+←`）。
